...
const simplifyUrl = (url)=>{
    return url.replace("https://", "").replace("http://", "") //注意用法,直接写上需要变换的value
    .replace("www.", "").replace(/\/.*/, "") //删除/开头的内容直到结尾
    ;
} //产生一个新的url,原本的url不变
;
const render = ()=>{
    $siteList.find("li:not(.last)").remove();
    /*清空之前的再重新渲染*/ hashMap.forEach((node, index)=>{
        //创建一个li  
        const $li = $(`<li>
...
$(".addButton").on("click", ()=>{
    ...
    if (url.indexOf("http") !== 0) url = "https://" + url; //用户输入非http开头的网址
    console.log(url);
    hashMap.push({
